...
// ⚑ THE LIVE CAR, not any car. A retired row is a car this Driver has sold; copying its
//   category would build a trip they are no longer eligible for (working_car(), S78).
const { data: vRows } = await db.from("vehicle").select("category,body_type").eq("driver_id", driver.id).is("retired_at", null).limit(1);
const car = vRows?.[0];
