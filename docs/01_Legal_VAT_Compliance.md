...
| **Cancellation — charged to the Business** | **follows whatever was cancelled** | ⚑ **ANSWERED 2026-09-04** (founder, [[d130]]): *"it works the same — apply the same rules on what was cancelled."* A cancelled transfer bills at the transfer's rate, a cancelled at-disposal block at the standard rate. It **delegates**, like waiting and no-show, so there is no second copy of the rule. ⚑ The source agrees: § 260 above taxes the retained price *« indépendamment »* of whether the client renounces the capacity **before** the date or fails to appear — it puts a cancellation and a no-show on the same footing. ⚑ **No Driver → *hors champ***, settled by the SQL not by opinion: `business_cancel_mission` zeroes the fee when nobody had accepted, so nothing was held for anyone |
