...
| ⚑ NOT applied yet | `2026-09-17_mission_client_writes.sql` ([[d144]], S82 parallel) — the founder pastes it, then `.local/probe/mission-client-writes/check.sql` (read-only, every row `pass`). Proof: `bash .local/probe/mission-client-writes/run.sh` on a throw-away PG17, 82/82 |
| tests | **1275** |
